...
def workspacerun(WorkSpaceID, ConfigVersionID, Token):
    ...
    payload1 = dict(data = dict(attributes={"is-destroy": False, "message": comment}, type="runs", relationships=dict(workspace=dict(data=dict(type="workspaces", id=WorkSpaceID)), configuration-version=dict(data=dict(type="configuration-versions", id=ConfigVersionID)))))
